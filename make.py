...
def make_nodebook(src_path, new_path, lang, remove_tag):
...
def make_nodebooks(src_file, lang, force_rebuild):
    versions = {
        'student': {'prefix': '', 'remove_tag': 'soln'},
        'teacher': {'prefix': 'solution-', 'remove_tag': 'exer'},
    }
    ...
    for version, attr in versions.items():
        dir_name = f"{attr['prefix']}{lang}"
    ...
    assert nb_cells['student'] == nb_cells['teacher'], \
        f'teacher and student versions have different number of cells ({lang})'
